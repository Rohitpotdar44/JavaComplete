...
public class TerminalOperations_3 {
    public static void main(String[] args) {
         List<Integer> list = Arrays.asList(1, 2, 3);

        // 1. collect
        list.stream().skip(1).collect(Collectors.toList()); // it makes use of collectors and then convert into list
        list.stream().skip(1).toList(); // or this is also ok 

        // 2. forEach
        list.stream().forEach(x -> System.out.println(x)); // apply for all ele

        // 3. reduce : Combines elements to produce a single result   ( combined into one result )   is optional comes in result then apply .get()
        Optional<Integer> optionalInteger = list.stream().reduce(Integer::sum); 
        System.out.println(optionalInteger.get()); //6

        // 4. count

        // 5. anyMatch, allMatch, noneMatch (these are shortcitcuit methods -> once val found stops)
        ...
        // Example: Squaring and Sorting Numbers ( we use map beacuse we have to take val and also return)
        ...
        // Example:  Counting Occurrences of a Character (sentence.chars() also converts into stream)
...
//        List<String> list1 = stream.map(String::toUpperCase).toList(); // exception
        ...
        // stateful (keep track of another ele in list or consideration).e.g sorted,distinct & stateless(not)
    }
}
