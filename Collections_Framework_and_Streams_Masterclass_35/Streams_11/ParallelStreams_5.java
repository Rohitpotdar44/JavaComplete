...
public class ParallelStreams_5 {
    public static void main(String[] args) {
        ...
        List<Long> factorialsList = list.stream().map(ParallelStreams_5::factorial).toList();
        long endTime = System.currentTimeMillis();
        System.out.println("Time taken with sequential stream: " + (endTime - startTime) + " ms");

        startTime = System.currentTimeMillis();
        factorialsList = list.parallelStream().map(ParallelStreams_5::factorial).toList();
...
/*
 Time taken with sequential stream: 283 ms
Time taken with parallel stream: 54 ms
Expected cumulative sum: [1, 3, 6, 10, 15]
Actual result with parallel stream: [1, 3, 6, 10, 15]
 */




 // soutc is the shortcut for method reference